...
ADMIN_IDS = [776338668, 748041053]  # ЗАМЕНИТЕ НА РЕАЛЬНЫЕ ID
...
#     """Показывает список отзывов на модерацию"""
#     pending_reviews = db.get_pending_reports()
#
#     if not pending_reviews:
#         await callback.message.edit_text(
#             "📭 <b>Нет отзывов на модерации</b>",
#             reply_markup=get_admin_main_keyboard(),
#             parse_mode=ParseMode.HTML
#         )
#         await callback.answer()
...
#         f"⏳ Ожидает проверки: <b>{len(pending_reviews)}</b>\n\n"
#         f"<i>Выберите отзыв для проверки:</i>",
...
#
#
# @router.callback_query(F.data.startswith("admin_reviews_page_"))
# async def paginate_reviews(callback: CallbackQuery):
#     """Пагинация списка отзывов"""
#     page = int(callback.data.split("_")[3])
#
#     await callback.message.edit_reply_markup(
#         reply_markup=get_admin_reviews_keyboard(page=page)
#     )
#     await callback.answer()
#
#
# @router.callback_query(F.data.startswith("admin_review_"))
# async def view_review(callback: CallbackQuery, state: FSMContext):
...
#         cursor = conn.cursor()
#         cursor.execute('''
#             SELECT r.*, t.last_name, t.first_name, t.patronymic
#             FROM ratings r
...
#
#     review = dict(row)
#     teacher_name = f"{review['last_name']} {review['first_name']}"
#     if review['patronymic']:
#         teacher_name += f" {review['patronymic']}"
#
#     tags = json.loads(review['tags']) if review['tags'] else []
#
#     review_text = f"<b>📝 Отзыв #{review_id}</b>\n\n"
#     review_text += f"<b>Преподаватель:</b> {teacher_name}\n"
#     review_text += f"<b>Оценка:</b> {'⭐' * int(review['score'])} {review['score']:.1f}/5.0\n\n"
#
#     if tags:
...
#
#     review_text += f"<b>Дата:</b> {review['created_at'][:16]}"
#
#     await state.update_data(current_review_id=review_id)
#
#     await callback.message.edit_text(
#         review_text,
...
#         parse_mode=ParseMode.HTML
#     )
#     await state.set_state(AdminStates.reviewing_review)
#     await callback.answer()
#
#
...
#             await state.set_state(AdminStates.viewing_reviews)
#         else:
#             await callback.answer("❌ Не удалось одобрить отзыв")
#     except Exception as e:
...
# async def reject_review(callback: CallbackQuery, state: FSMContext):
#     """Отклонение отзыва"""
#     review_id = int(callback.data.split("_")[2])
#
#     try:
#         success = db.reject_review(review_id)
#
#         if success:
...
#             await state.set_state(AdminStates.viewing_reviews)
#         else:
#             await callback.answer("❌ Не удалось отклонить отзыв")
#     except Exception as e:
...
#                 f"👨‍🏫 Преподаватель: {teacher_name}\n"
#                 f"⭐ Оценка: {score:.1f}/5.0\n\n"
#                 f"Нажмите кнопку ниже, чтобы перейти к модерации:",
#                 reply_markup=get_admin_review_notification_keyboard(review_id),
#                 parse_mode=ParseMode.HTML
#             )
#         except Exception as e:
#             logger.error(f"Failed to notify admin {admin_id}: {e}")
